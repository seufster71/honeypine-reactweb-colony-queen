...
export default function CCHiveView({itemState, appPrefs, onListLimitChange,
	onSearchChange, onSearchClick, onPaginationClick, onOrderBy, onOption,
	closeModal, inputChange, goBack, session}) {

    let columns = [];
    if (itemState.prefLabels != null && itemState.prefLabels.CC_HIVE_PAGE != null) {
    	columns = itemState.prefLabels.CC_HIVE_PAGE;
    }
    let group = "TABLE1";
	...
	if (itemState.prefTexts.CC_HIVE_PAGE != null && itemState.prefTexts.CC_HIVE_PAGE.CC_HIVE_PAGE_HEADER != null) {
		header = itemState.prefTexts.CC_HIVE_PAGE.CC_HIVE_PAGE_HEADER.value;
	}
