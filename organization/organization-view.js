...
export default function CCOrganizationView({itemState, appPrefs, onListLimitChange,
	onSearchChange, onSearchClick, onPaginationClick, onOrderBy, onOption, 
	closeModal, session}) {

    let columns = [];
    if (itemState.prefLabels != null && itemState.prefLabels.CC_ORGANIZATION_PAGE != null) {
    	columns = itemState.prefLabels.CC_ORGANIZATION_PAGE;
    }
    let group = "TABLE1";
    
    let header = "";
	if (itemState.prefTexts.CC_ORGANIZATION_PAGE != null && itemState.prefTexts.CC_ORGANIZATION_PAGE.CC_ORGANIZATION_PAGE_HEADER != null) {
		header = itemState.prefTexts.CC_ORGANIZATION_PAGE.CC_ORGANIZATION_PAGE_HEADER.value;
	}
